...
|---|---|---|---|---|---|---|---|---|
EOF
		if  [[ $type == "per" ]] && ls "${expdir}/*/*/score_per/scoring/*.filt.sys"  &> /dev/null; then
	    		## If STM used for HUBSCR based scoring, the *.sys files have the per, not result.txt or result.wrd.txt
            		grep -H -e Sum/Avg "${expdir}"/*/*/score_per/scoring/*.filt.sys \
				| sed -e "s#${expdir}/\([^/]*/[^/]*\)/score_per/scoring/\([[:graph:]]*\):#|\1/\2#g" \
