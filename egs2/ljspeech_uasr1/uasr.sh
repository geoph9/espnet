...
train_set=tr_no_dev       # Name of training set.
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        if "${use_ngram}"; then
            ...
            _logdir="${ngram_exp}"
            # If it fails here make sure KENLM is installed. If you have an issue with the cmake version then download the cmake
            # tarball and use that (check install_kenlm.sh in tools for the steps).
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        if "${use_k2}"; then
            ...
            # MYNOTE: this export needs to be added in order to avoid a segmentation fault.
            #         See: https://github.com/k2-fsa/icefall/issues/674
    ...
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
        ...
        if ! "${write_collected_feats}"; then
            # MYNOTE: The following for-loop and input_dim calculation is done by me since the input size 
            #         (i.e. #mfcc features) is required by uasr_train (for the generator input).
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                if [ "${dset}" = "${train_set}" ] || [ "${dset}" = "${valid_set}" ]; then
                    _suf="/org"
                else
                ...
                if [ ! -f "${data_feats}${_suf}/${dset}/feats_dim" ] ; then
                  # Derive the the frame length and feature dimension
                  _nj=$(min "${nj}" "$(<"${data_feats}${_suf}/${dset}/utt2spk" wc -l)")
                  scripts/feats/feat_to_shape.sh --nj "${_nj}" --cmd "${train_cmd}" \
                      "${data_feats}${_suf}/${dset}/feats.scp" "${data_feats}${_suf}/${dset}/feats_shape"
    ...
    if [ ${stage} -le 15 ] && [ ${stop_stage} -ge 15 ]; then
        ...
        if [ "${_feats_type}" = raw ]; then
            if "${write_collected_feats}"; then
                ...
                # MYNOTE: Make sure that input size is taken from a valid feats_dim
                if [ -f "${uasr_stats_dir}/train/feats_dim" ] ; then
                    _input_size="$(<${uasr_stats_dir}/train/feats_dim)"
                    _opts+="--input_size=${_input_size} "  # if it doesn't exist then pass None as input_size
                fi
            else
                _scp=wav.scp
                # "sound" supports "wav", "flac", etc.
